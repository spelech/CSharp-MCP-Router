...
![Version](https://img.shields.io/badge/version-v5.6.1-orange?style=for-the-badge)
...
| :--- | :--- | :--- |
| **`v5.6.1`** | 2026-09-02 | chore(refactor): Fixed empty catch blocks in SseTransport `SendRequestAsync` and `CallMethodAsync` by safely logging `OperationCanceledException` at Debug level and other exceptions at Warning level without crashing the transport. Added robust test coverage `TRANS-01` to ensure correct handling of cancellation timeouts. |
| **`v5.6.0`** | 2026-09-01 | feat(mcp): MCP 2026-07-28 Spec Alignment — OpenTelemetry Trace Context Propagation, Subscriptions/Listen Stream, and Stateless Request Validation. Implements end-to-end OpenTelemetry and W3C trace context extraction and injection (`traceparent`, `tracestate`, `baggage`, `_meta.io.modelcontextprotocol/trace`) across `McpSpecMiddleware`, JSON-RPC request rewriter, and downstream HTTP/SSE backend transports; implements MCP 2026-07-28 `subscriptions/listen` request handler and updates server capabilities across all endpoints (`/sse`, `/mcp`, `/admin`, `/mcg-admin`, `/{targetServerId}`) to remove legacy `resources.subscribe = false` and advertise `subscriptions = {}`; adds stateless request execution without prior `initialize` / `notifications/initialized` handshake requirements; adds protocol version negotiation and validation returning standard JSON-RPC error `-32021` (`UnsupportedProtocolVersionError`) for invalid requested versions; and expands living requirements catalog (`MCP-22`, `MCP-23`, `MCP-24`) to 184 requirements across 830 proofs. |
