...
3. **Docker / Kubernetes File Secret (`MCG_MASTER_KEY_FILE` / `ROUTER_MASTER_KEY_FILE` - `KeySource.External`)**:
   Point `MCG_MASTER_KEY_FILE` at a mounted secret file (e.g., `MCG_MASTER_KEY_FILE=/run/secrets/mcg_master_key`). The gateway also automatically checks standard Docker secret paths (`/run/secrets/mcg_master_key`, `/run/secrets/master_key`, `/run/secrets/router_master_key`).
4. **Explicit Environment Variable (`MCG_MASTER_KEY` / `ROUTER_MASTER_KEY` - `KeySource.External`)**:
   Pass a 256-bit base64-encoded key directly via `MCG_MASTER_KEY=<base64-key>` (or fallback `ROUTER_MASTER_KEY`) if preferred.
...
| **🔑 Admin AppKey** | • Seeds a default system Admin AppKey into the database: `mcp-adm-` or configured `MCG_ADMIN_KEY` / `ROUTER_ADMIN_KEY`.<br>• Has username `admin` and scope `["all", "admin"]`.<br>• Enables remote AI coding agents and CLI scripts to authenticate to `/admin` and `/admin/sse` immediately. |
...
| :--- | :--- | :--- | :--- |
| **`MCG_ADMIN_KEY`** | `MCG_ADMIN_KEY` (or `ROUTER_ADMIN_KEY`) | Optional | Custom admin token or compact Base62 key (e.g. `mcp-adm-...`) seeded for user `admin`. |
| **`MCG_MASTER_KEY`** | `MCG_MASTER_KEY` (or `ROUTER_MASTER_KEY` / `MCG_MASTER_KEY_FILE`) | Optional | High-entropy 256-bit key or file path. If unset, automatically generated and stored in `./data/.master.key`. |
