...
namespace ModelContextGateway.Tests
{
    public class TokenExchangeSecretRetrieverTests
    {
        ...
        [Fact]
        public async Task GetSecretAsync_ValidatesIssParameter_SuccessWhenMatched()
        {
            var tokenResponse = new
            {
            ...
            var configDict = new Dictionary<string, string?>
            {
                ["Identity:TokenExchange:Issuer"] = "https://identity.local/",
                ["Identity:TokenExchange:TokenEndpoint"] = "https://identity.local/oauth/token",
                ["Identity:TokenExchange:ClientId"] = "client123"
            };
            var config = new ConfigurationBuilder().AddInMemoryCollection(configDict).Build();
            ...
            var token = await retriever.GetSecretAsync("", "mcp:read");
            Assert.Equal("valid_token_123", token);
        }

        [Fact]
        public async Task GetSecretAsync_ThrowsSecurityException_WhenIssParameterMismatchesExpectedIssuer()
        {
            var tokenResponse = new
            {
            ...
            var ex = await Assert.ThrowsAsync<System.Security.SecurityException>(() => retriever.GetSecretAsync("", "mcp:read"));
            Assert.Contains("issuer mismatch", ex.Message);
        }
