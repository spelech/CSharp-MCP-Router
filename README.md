...
| :--- | :--- | :--- |
| **`v3.0.1`** | 2026-08-07 | **Performance Optimization: Eliminated SSE Polling Loop.** Refactored backend SSE connection logic (`SseTransport.cs`) to eliminate inefficient `Task.Delay` CPU-polling loops. Introduced a non-blocking, thread-safe `TaskCompletionSource<string>` (`_messageUrlTcs`) to await the downstream endpoint URL asynchronously with `Task.WaitAsync` (5-second timeout). This drastically reduces connection establishment latency and CPU resource consumption. |
