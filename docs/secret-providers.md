...
2. **Environment Variables (`KeySource.External`)**:
   `MCG_MASTER_KEY`, `ROUTER_MASTER_KEY`, `ROUTER_SECRET`, or `DB_ENCRYPTION_KEY`.
3. **Secret File Mount (`KeySource.External`)**:
   `MCG_MASTER_KEY_FILE`, `ROUTER_MASTER_KEY_FILE`, or standard Docker/Kubernetes secret mounts (`/run/secrets/mcg_master_key`, `/run/secrets/router_master_key`, `/run/secrets/master_key`).
...
### Declarative Admin AppKey Seeding (`MCG_ADMIN_KEY` / `ROUTER_ADMIN_KEY`)

The gateway supports declarative admin key provisioning:
- **Environment Variables**: `MCG_ADMIN_KEY`, `ROUTER_ADMIN_KEY`, or `MCP_ADMIN_KEY`.
