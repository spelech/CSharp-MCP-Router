...
**Implementation**: [`SymmetricEncryptionHelper.cs`](file:///containers/dev/csharp-mcp-router/.worktrees/issue-56/Infrastructure/Secrets/SymmetricEncryptionHelper.cs)
...
1. **Vault Master Key Bootstrapping (`KeySource.External`)**:
   In enterprise multi-replica clusters, if `VAULT_ADDR` is present, the router connects to HashiCorp Vault (via AppRole, Token, or Kubernetes Service Account auth) and fetches `secret/data/mcp-router/master-key`.
...
3. **Secret File Mount (`KeySource.External`)**:
   `ROUTER_MASTER_KEY_FILE` or standard Docker/Kubernetes secret mounts (`/run/secrets/router_master_key`, `/run/secrets/master_key`).
...
5. **Auto-Generated Persistent Keyfile (`KeySource.AutoGenerated`)**:
   If no master key is supplied on first boot, the router automatically generates a cryptographically secure 256-bit key and persists it to `./data/.master.key` (with restricted permissions `chmod 0600`).
...
### Declarative Admin AppKey Seeding (`ROUTER_ADMIN_KEY`)

The router supports declarative admin key provisioning:
- **Environment Variables**: `ROUTER_ADMIN_KEY` or `MCP_ADMIN_KEY`.
...
The following `docker-compose.yaml` provisions `mcp-router` connected to a local `vault` container with persistent data and mutual networking:
...
services:
  ...
  mcp-router:
    ...
    environment:
      ...
      - ROUTER_MASTER_KEY=SuperSecretDeploymentMasterKey2026!MustBe32CharsLong
...
Run these commands inside your Vault container to initialize the KV v2 secrets engine, create a least-privilege policy, and generate AppRole credentials for the router:
...
# 3. Create least-privilege policy for MCP Router
cat << 'EOF' > /tmp/mcp-router-policy.hcl
path "secret/data/*" {
  capabilities = ["read"]
}
...
vault policy write mcp-router /tmp/mcp-router-policy.hcl

# 4. Enable and configure AppRole
vault auth enable approle
vault write auth/approle/role/mcp-router-role \
    token_policies="mcp-router" \
    token_ttl=1h \
    token_max_ttl=24h

# 5. Fetch Role ID and Secret ID
ROLE_ID=$(vault read -field=role_id auth/approle/role/mcp-router-role/role-id)
SECRET_ID=$(vault write -f -field=secret_id auth/approle/role/mcp-router-role/secret-id)
...
| **`FATAL: Master encryption key is missing.`** | Neither `ROUTER_MASTER_KEY` nor `DB_ENCRYPTION_KEY` is defined. | Define `ROUTER_MASTER_KEY` in `docker-compose.yaml` or `.env`. Ensure it is a persistent, non-empty secret string. |
