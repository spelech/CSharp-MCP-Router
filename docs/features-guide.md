...
- **Declarative Seeding**: Administrators can seed custom admin keys on startup via `ROUTER_ADMIN_KEY` or `MCP_ADMIN_KEY`.
...
1. **Key Source Detection (`KeySource`)**:
   - `KeySource.External`: Provided via HashiCorp Vault bootstrapping (`VAULT_ADDR`), container environment variable (`ROUTER_MASTER_KEY`), or secret file mount (`ROUTER_MASTER_KEY_FILE`). Configuration changes in Web UI are locked.
