...
namespace McpRouter.Infrastructure.Identity
{
    public class LdapActiveDirectoryService : ILdapService
    {
        ...
        public async Task<List<string>> ResolveUserSidsAsync(string username)
        {
            ...
            // 1. Try to load enabled ActiveDirectory provider from database repository
            if (_authRepo != null)
            {
                try
                {
                    var dbProviders = await _authRepo.GetAuthProvidersAsync();
                    var adDb = dbProviders?.FirstOrDefault(p =>
                    ...
                    if (adDb != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(adDb.ConfigJson))
                        {
                            ...
                            if (root.TryGetProperty("server", out var sProp) || root.TryGetProperty("ldap_server", out sProp))
                            {
