...
**Implementation**: [`DbKeyHelper.cs`](file:///containers/dev/csharp-mcp-router/.worktrees/issue-56/Infrastructure/Secrets/DbKeyHelper.cs)

The 256-bit symmetric encryption key is derived deterministically from the master deployment secret using Password-Based Key Derivation Function 2 (PBKDF2):

1. **Master Secret Resolution**:
   The router retrieves the master secret from the environment in order of precedence:
   - `ROUTER_SECRET`
   - `ROUTER_MASTER_KEY`
   - `DB_ENCRYPTION_KEY`
   > [!CAUTION]
   > For production security, ephemeral fallback keys are strictly disabled. If no master key is supplied in the environment, the router throws a fatal `InvalidOperationException` on startup to prevent encrypting data with transient keys.
