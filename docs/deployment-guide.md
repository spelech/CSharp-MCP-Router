...
1. **Vault Master Key Bootstrapping (`KeySource.External`)**:
   When deploying in an enterprise multi-replica cluster with `VAULT_ADDR` configured, the gateway boots its master key directly from HashiCorp Vault (`secret/data/mcp-router/master-key`).
...
3. **Docker / Kubernetes File Secret (`ROUTER_MASTER_KEY_FILE` - `KeySource.External`)**:
...
4. **Explicit Environment Variable (`ROUTER_MASTER_KEY` or `ROUTER_SECRET` - `KeySource.External`)**:
...
| **🔑 Admin AppKey** | • Seeds a default system Admin AppKey into the database: `mcp-adm-` or configured `ROUTER_ADMIN_KEY` (fallback: `mcp-global-admin-default-cli-key-99`).<br>• Has username `admin` and scope `["all", "admin"]`.<br>• Enables remote AI coding agents and CLI scripts to authenticate to `/admin` and `/admin/sse` immediately. |
...
* **Health Probe**: `http://localhost:8080/health` (`{"status":"healthy","service":"McpRouter","version":"4.35.0"}`)
...
| :--- | :--- | :--- | :--- |
| **`ROUTER_ADMIN_KEY`** | `ROUTER_ADMIN_KEY` or `MCP_ADMIN_KEY` | Optional | Custom admin token or compact Base62 key (e.g. `mcp-adm-...`) seeded for user `admin`. |
